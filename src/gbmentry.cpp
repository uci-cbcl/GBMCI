...
{
	//cout<<"In GBM entry function!"<<flush<<endl;
    ...
    if(ISNA(REAL(radFOld)[0])) // check for old predictions
    {
        ...
        {
        	if (strncmp(CHAR(STRING_ELT(rszFamily,0)),"sci",2) != 0)
        		REAL(radF)[i] = REAL(rdInitF)[0];
        	else
        	{
        		REAL(radF)[i] = REAL(rdInitF)[0]+unif_rand();
        		//Rprintf("Random initialization for sci! \n");
        	}
        }
        // CYF
    	// Rprintf("const initialization! \n");
        //Rprintf("#data: %d , initial prediction: %f \n",
        //		pData->cRows, REAL(rdInitF)[0]);
    }
    else
    {
    	// CYF
    	//Rprintf("\ncoxph initialization for sci model! \n");
        for(i=0; i < pData->cRows; i++)
        {
            REAL(radF)[i] = REAL(radFOld)[i];
            //Rprintf("%f, ",REAL(radF)[i]);
        }
        //Rprintf("\n");
    }
    ...
    {
    	// Rprintf("pGBM iterate \n");
    	// for (int ii=0; ii<)
        hr = pGBM->iterate(REAL(radF),
                           dTrainError,dValidError,dOOBagImprove,
                           cNodes);
        // Rprintf("pGBM iterate finish \n");
        if(GBM_FAILED(hr))
        {
        	Rprintf("pGBM iterate fail \n");
            goto Error;
        }
        ...
        //Rprintf("Iteration %d finished \n", iT);
    }
    if(INTEGER(rfVerbose)[0]) Rprintf("\n");

    //Rprintf("right after tree iterations are finished\n");
    ...
    //Rprintf("right after split are finished\n");
...
Cleanup:
    ...
    //Rprintf("right before general return\n");
...
)
{
   ...
   if(!fSingleTree)
   {
	   // Rprintf("prediction !fSingleTree\n"); // this way // CYF
